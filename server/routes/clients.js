import express from "express";
import mongoose from "mongoose";
import Client from "../models/Client.js";
import Payment from "../models/Payment.js";
import { requireUser } from "../middleware/auth.js";
import { clientHistory } from "../lib/clientIdentity.js";
import { toPlain } from "../lib/agencyData.js";
import {
  cargoCustomerCharge,
  deriveCustomerFinanceSummary,
} from "../lib/finance.js";

const router = express.Router();

router.get("/:id/history", requireUser, async (req, res) => {
  const clauses = [{ id: req.params.id }];
  if (mongoose.isValidObjectId(req.params.id)) clauses.push({ _id: req.params.id });
  const client = await Client.findOne({ $or: clauses });
  if (!client) return res.status(404).json({ error: "Client not found." });
  const history = await clientHistory(client._id, req.user);
  const transactionKeys = [
    ...history.tickets.map((row) => ["ticket", row.id]),
    ...history.visas.map((row) => ["visa", row.id]),
    ...history.cargo.map((row) => ["cargo", row.id]),
  ];
  const payments = transactionKeys.length
    ? await Payment.find({
        $or: transactionKeys.map(([transactionType, transactionId]) => ({
          transactionType,
          transactionId,
        })),
      }).lean()
    : [];
  const paymentsByTransaction = new Map();
  for (const payment of payments) {
    const key = `${payment.transactionType}:${payment.transactionId}`;
    paymentsByTransaction.set(key, [
      ...(paymentsByTransaction.get(key) || []),
      payment,
    ]);
  }
  const withFinance = (transactionType, document) => {
    const record = toPlain(document);
    const summary = deriveCustomerFinanceSummary({
      totalCharge:
        transactionType === "cargo"
          ? cargoCustomerCharge(record)
          : record.type === "Refund"
            ? 0
            : Number(record.amount) || 0,
      payments:
        paymentsByTransaction.get(`${transactionType}:${record.id}`) || [],
    });
    return {
      ...record,
      ...summary,
      amountPaid: summary.totalPaid,
      balance: summary.balanceDue,
    };
  };
  return res.json({
    client: toPlain(client),
    tickets: history.tickets.map((row) => withFinance("ticket", row)),
    visas: history.visas.map((row) => withFinance("visa", row)),
    cargo: history.cargo.map((row) => withFinance("cargo", row)),
  });
});

export default router;

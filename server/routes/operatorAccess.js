...
import { operatorSettings, operatorAccessPayload, validateOperatorRoute, generateOperatorRoute, parseOperatorAddress } from "../lib/operatorAccess.js";
...
router.patch("/", requireOwner, async (req, res) => {
  const update = req.body?.regenerate === true ? { operatorAccessRoute: generateOperatorRoute() }
    : req.body?.address !== undefined ? parseOperatorAddress(req.body.address)
    : { operatorAccessRoute: validateOperatorRoute(req.body?.route) };
  await operatorSettings();
  const settings = await AgencySettings.findOneAndUpdate({ key: "singleton" }, { $set: update }, { new: true }).lean();

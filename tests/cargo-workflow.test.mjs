...
import {
  assertCargoTransitionAllowed,
  canViewCargo,
  normalizeCargoStatus,
  transitionCargoStatus,
} from "../server/lib/cargoWorkflow.js";
...
const mogadishu = {
  id: "op-mog",
  name: "Mogadishu Operator",
  role: "operator",
  assignedBranchId: mog,
};
const hargeisa = {
  id: "op-hga",
  name: "Hargeisa Operator",
  role: "operator",
  assignedBranchId: hga,
};
...
const withCargoMocks = async (cargo, fn) => {
  ...
  const originalCargoFindOneAndUpdate = Cargo.findOneAndUpdate;
  const originalCargoFind = Cargo.find;
  const originalBranchFindOne = Branch.findOne;
  Cargo.findOne = async (query) =>
    cargo.find((item) => item.id === query.id) || null;
  Cargo.findOneAndUpdate = async (query, update) => {
    const item = cargo.find(
      (row) =>
        row.id === query.id &&
        row.status === query.status &&
        (row.workflowVersion || 0) === query.workflowVersion,
    );
    if (!item) return null;
    Object.assign(item, update.$set || {});
    item.workflowVersion =
      (item.workflowVersion || 0) + (update.$inc?.workflowVersion || 0);
    if (update.$push?.statusHistory) {
      item.statusHistory.push(update.$push.statusHistory);
    }
    return item;
  };
  ...
  try {
    ...
    Cargo.findOneAndUpdate = originalCargoFindOneAndUpdate;
...
test("valid cargo lifecycle transitions are accepted", () => {
  const cargo = {
    status: "received",
    originBranchId: nbo,
    destinationBranchId: mog,
  };
  assert.doesNotThrow(() =>
    assertCargoTransitionAllowed(cargo, "in_transit", nairobi),
  );
  cargo.status = "in_transit";
  assert.doesNotThrow(() =>
    assertCargoTransitionAllowed(cargo, "arrived", mogadishu),
  );
  cargo.status = "arrived";
  assert.doesNotThrow(() =>
    assertCargoTransitionAllowed(cargo, "ready_for_collection", mogadishu),
  );
  assert.doesNotThrow(() =>
    assertCargoTransitionAllowed(cargo, "delivered", mogadishu),
  );
...
test("invalid cargo lifecycle transitions are rejected", () => {
  ...
  assert.throws(
    () => assertCargoTransitionAllowed(cargo, "arrived", nairobi),
    /cannot move/,
  );
  assert.throws(
    () => assertCargoTransitionAllowed(cargo, "delivered", nairobi),
    /cannot move/,
  );
  cargo.status = "in_transit";
  assert.throws(
...
test("origin and destination operators have separate cargo action authority", () => {
  ...
  assert.throws(
    () => assertCargoTransitionAllowed(cargo, "in_transit", mogadishu),
    /origin branch/,
  );
  cargo.status = "in_transit";
  assert.doesNotThrow(() =>
  ...
  assert.throws(
    () => assertCargoTransitionAllowed(cargo, "delivered", hargeisa),
    /destination branch/,
  );
  assert.doesNotThrow(() =>
    assertCargoTransitionAllowed(
...
test("third-branch workflows are authorized without source-code branch coupling", () => {
  const hgaToNbo = {
    status: "received",
    originBranchId: hga,
    destinationBranchId: nbo,
  };
  assert.doesNotThrow(() =>
    assertCargoTransitionAllowed(hgaToNbo, "in_transit", hargeisa),
  );
  hgaToNbo.status = "in_transit";
  assert.doesNotThrow(() =>
    assertCargoTransitionAllowed(hgaToNbo, "arrived", nairobi),
  );
  hgaToNbo.status = "arrived";
  assert.doesNotThrow(() =>
    assertCargoTransitionAllowed(hgaToNbo, "delivered", nairobi),
  );

  const nboToHga = {
    status: "received",
    originBranchId: nbo,
    destinationBranchId: hga,
  };
  assert.doesNotThrow(() =>
    assertCargoTransitionAllowed(nboToHga, "in_transit", nairobi),
  );
  nboToHga.status = "in_transit";
  assert.doesNotThrow(() =>
    assertCargoTransitionAllowed(nboToHga, "arrived", hargeisa),
  );
});
...
test("incoming cargo is visible through one shared cargo record", () => {
  ...
  assert.deepEqual(
    rows.filter((item) => canViewCargo(item, mogadishu)).map((item) => item.id),
    ["nbo-mog", "hga-mog"],
  );
});
...
test("transition service records ordered status history with actor, time, and branch", async () => {
  ...
  assert.deepEqual(
    cargo.statusHistory.map((entry) => entry.toStatus),
    ["in_transit", "arrived", "ready_for_collection", "delivered"],
  );
